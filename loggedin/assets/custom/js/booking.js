...
$(document).on('click', '.add-more-timeshedule', function () {
    ...
    data += '<button class="btn btn-primary publish-btn">Save</button>	<button class="cancel-pop-items btn save-cancel-btn data';
    data += 'cancel-with-border">Cancel</button>';
    data += '</div>';
    data += '</div>';
    data += '</div>';
    data += '</div>';
...
$('.booking-accordian').on('click', '.cat-edit', function () {
...
$(".toggleDescr").click(function () {
    ...
    $(this).closest('.portlet-body').find('.descr_section').toggleClass("hide");
});
...
function copyClass(a, b)
{
    if (a) {
        $('#loading-overlay').show();
        var setvice_name = $("#title" + a).text();
        $.post('copyClass', {a: a, service_name: setvice_name}, function (data1) {
            setTimeout(function () {
                $("#sortable" + b).load('class_list' + " #sortable" + b);
...
copyBus
function copyBus(a, b)
{
    if (a) {
        $('#loading-overlay').show();
        var setvice_name = $("#title" + a).text();
        $.post('copyBus', {a: a, service_name: setvice_name}, function (data1) {
            setTimeout(function () {
                $("#sortable" + b).load('buslist' + " #sortable" + b);
                $('#loading-overlay').hide();
            }
            , 2000);
            return false;
...
function copyServices(a, b)
{
    if (a) {
        $('#loading-overlay').show();
        var setvice_name = $("#title" + a).text();
        $.post('copyServices', {a: a, service_name: setvice_name}, function (data1) {
            setTimeout(function () {
                $("#sortable" + b).load('appoinment_list' + " #sortable" + b);
                $('#loading-overlay').hide();
            }
            , 2000);
            return false;
...
{
    $('#service' + a).removeClass("sr-msg-error");
    var service = $('#service' + a).val();
    if (service) {
        $.post('updateCategory', {a: a, name: service}, function (data1) {// alert(data1);

            if (data1 == 0) {
                $('#service' + a).addClass("sr-msg-error");
                $('#error' + a).html("The name '" + service + "' is already taken. Please choose a differnt name.");
            } else {
                $('#panel_' + a).closest('.booking-panel').find('.portlet-title').html(service);
                $('#panel_' + a).closest('.booking-panel').find('.portlet-title').removeClass('hide');
                $('#panel_' + a).closest('.booking-panel').find('.update-category').addClass('hide');
                $('#panel_' + a).closest('.booking-panel').find('.update-category-blank').removeClass('hide');
            }
            return false;
        }
        );
...
// booking appointment add edit  new_appoinment page 
 $("#allstaff").click(function() {
        $('input:checkbox').not(this).prop('checked', this.checked);
    });
    
     $('#add_staff_submit').click(function() {
        var add_staff_name = $('#new_staff').val();
        if (add_staff_name != '') {
            $.ajax({
                type: "POST",
                url: "ajx_new_staff",
                ...
                success: function(data) {
                  ///   alert(data);
                    $('#append_checkbox').append(data);
                    $('#new_staff').val("");
                    $(".cancel-staff-list").click();
        ...
        function bookingSubmit(a) {
            $("#booking_status").val(a);
            submit_form('bookingForm', 'apt_res');
        } 
